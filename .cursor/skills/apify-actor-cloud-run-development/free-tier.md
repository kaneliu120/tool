# Free-tier limits + worker config (templates)

Parent skill: [SKILL.md](SKILL.md). This file holds code templates, README/schema copy,
unit-test patterns, and anti-tamper notes formerly in `apify-actor-free-tier-limits`.
...
#    python3 "$HOME/.cursor/skills/apify-actor-cloud-run-development/scripts/random_smoke_input.py" \
#      --matrix ./coverage-matrix.json --out /tmp/input.json
#    Keep maxResults 2–5. If empty, re-roll — do not fall back to the docs example.
...
def worker_provides_proxy() -> bool:
    """Actor skips Apify RESIDENTIAL; worker uses egress-control / PROXY_URL."""
...
# New Actors: default WORKER_PROVIDES_PROXY=1 (egress-control on worker).
use_worker_proxy = worker_provides_proxy()
if use_worker_proxy:
    proxy_url = ""
    payload = config.to_worker_payload(proxy_url="")
    payload.pop("proxyUrl", None)  # worker PROXY_URL / egress-applied
else:
    # Legacy / explicit debug only — not the new-Actor default
...
| 4 | Default: `WORKER_PROVIDES_PROXY=1` + worker egress-control / `PROXY_URL` fallback |
